"""Tests for CLI commands — wrap logging, scan report mode, and basic commands."""
...
# ---------------------------------------------------------------------------
# Helper — use REAL secret patterns that the scanner will actually detect
# ---------------------------------------------------------------------------
...
class TestScanDefault:
    ...
    def test_scan_stdin_clean(self, runner):
...
class TestWrapLogging:
    ...
    def test_wrap_shows_log_path(self, runner, tmp_path):
        ...
        with patch("subprocess.Popen") as mock_popen, patch(
            ...
            "secretgate.cli._find_available_port", side_effect=[19999, 20000, 20001]
        ):
            proc = MagicMock()
    ...
    def test_wrap_help_shows_env_var(self, runner):
        """SECRETGATE_LOG_FILE env var should be documented in help."""
        result = runner.invoke(main, ["wrap", "--help"])
        assert "SECRETGATE_LOG_FILE" in result.output

    def test_wrap_popen_uses_pipe_not_devnull(self, runner, tmp_path):
        """Popen should use PIPE (for tee) not DEVNULL."""
        log_path = tmp_path / "test.log"

        with patch("subprocess.Popen") as mock_popen, patch(
            "subprocess.run"
        ) as mock_run, patch(
            "secretgate.cli._find_available_port", side_effect=[19998, 19999, 20000]
        ):
            proc = MagicMock()
            proc.poll.return_value = None
            proc.pid = 12345
